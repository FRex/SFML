...
bool Font::openFromFile(const std::filesystem::path& filename)
{
    ...
    auto        stream = std::make_shared<sf::FileInputStream>();
    ...
    const char* type   = "android resource stream";

#endif

    // Open the font, and if succesful save the stream to keep it alive
    if (renameThisSomeone(*stream, type))
    {
        m_stream = stream;
        return true;
    }

    // If loading failed, print filename (after the error message already printed in renameThisSomeone)
...
bool Font::openFromMemory(const void* data, std::size_t sizeInBytes)
{
    ...
    // Open the font, and if succesful save the stream to keep it alive
    if (renameThisSomeone(*memorystream, "memory"))
    {
...
bool Font::openFromStream(InputStream& stream)
{
    ...
    return renameThisSomeone(stream, "stream");
}
...
////////////////////////////////////////////////////////////
bool Font::renameThisSomeone(InputStream& stream, const char* type)
{
    // Cleanup the previous resources
    cleanup();
